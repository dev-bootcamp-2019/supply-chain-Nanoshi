...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  modifier verifySeller(uint _sku, address _seller){require(items[_sku].seller == _seller);_;}
  modifier verifyBuyer(uint _sku, address _buyer){require(items[_sku].buyer == _buyer);_;}
  ...
  modifier sold(uint _sku) { require(items[_sku].state == State.Received);_;}
  ...
  function shipItem(uint sku)
    public sold(sku) verifySeller(sku, msg.sender) 
  {
      emit Shipped(sku);
  ...
  function receiveItem(uint sku)
    public shipped(sku) verifyBuyer(sku, msg.sender)
  {
      emit Received(sku);
